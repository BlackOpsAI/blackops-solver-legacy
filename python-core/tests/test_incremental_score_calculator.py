...
def test_constraint_match_disabled_incremental_score_calculator():
    class NQueensIncrementalScoreCalculator(IncrementalScoreCalculator):
        ...
        def insert(self, queen: Queen):
            if queen.row is not None:
                ...
                ascending_diagonal_index_list = self.ascending_diagonal_index_map[
                    queen.get_ascending_diagonal_index()
                ]
                self.score -= len(ascending_diagonal_index_list)
                ascending_diagonal_index_list.append(queen)
                descending_diagonal_index_list = self.descending_diagonal_index_map[
                    queen.get_descending_diagonal_index()
                ]
    ...
    for i in range(4):
        for j in range(i + 1, 4):
            ...
            assert (
                left_queen.get_ascending_diagonal_index()
                != right_queen.get_ascending_diagonal_index()
            )
            assert (
                left_queen.get_descending_diagonal_index()
                != right_queen.get_descending_diagonal_index()
            )


@pytest.mark.skip(
    reason="Special case where you want to convert all items of the list before returning."
...
def test_constraint_match_enabled_incremental_score_calculator():
    ...
    class NQueensIncrementalScoreCalculator(
        ...
        def reset_working_solution(
            self, working_solution: Solution, constraint_match_enabled=False
        ):
            n = working_solution.n
            self.row_index_map = dict()
            self.ascending_diagonal_index_map = dict()
        ...
        def get_constraint_match_totals(self):
            row_conflict_constraint_match_total = DefaultConstraintMatchTotal(
                "NQueens", "Row Conflict", SimpleScore.ONE
            )
            ascending_diagonal_constraint_match_total = DefaultConstraintMatchTotal(
                "NQueens", "Ascending Diagonal Conflict", SimpleScore.ONE
            )
            descending_diagonal_constraint_match_total = DefaultConstraintMatchTotal(
                "NQueens", "Descending Diagonal Conflict", SimpleScore.ONE
            )
            for row, queens in self.row_index_map.items():
                if len(queens) > 1:
                    row_conflict_constraint_match_total.addConstraintMatch(
                        queens, SimpleScore.of(-len(queens) + 1)
                    )
            for row, queens in self.ascending_diagonal_index_map.items():
                if len(queens) > 1:
                    ascending_diagonal_constraint_match_total.addConstraintMatch(
                        queens, SimpleScore.of(-len(queens) + 1)
                    )
            for row, queens in self.descending_diagonal_index_map.items():
                if len(queens) > 1:
                    descending_diagonal_constraint_match_total.addConstraintMatch(
                        queens, SimpleScore.of(-len(queens) + 1)
                    )
            return [
    ...
    constraint_match_total_map = score_manager.explain(
        solution
    ).constraint_match_total_map
    row_conflict = constraint_match_total_map.get("NQueens/Row Conflict")
    ascending_diagonal_conflict = constraint_match_total_map.get(
        "NQueens/Ascending Diagonal Conflict"
    )
    descending_diagonal_conflict = constraint_match_total_map.get(
        "NQueens/Descending Diagonal Conflict"
    )
...
def test_error_message_for_missing_methods():
    with pytest.raises(
        TypeError
    ):  # Exact error message from ABC changes between versions
