...
def my_constraints(constraint_factory: ConstraintFactory):
    return [
        ...
        .join(
            Value, Joiners.equal(lambda entity: entity.value, lambda value: value.code)
        )
...
def test_load_from_solver_config_file():
    ...
    assert (
        solver_config.getSolutionClass()
        == get_java_type_for_python_type(Solution).getJavaClass()
    )
    entity_class_list = solver_config.getEntityClassList()
    assert entity_class_list.size() == 1
    assert (
        entity_class_list.get(0) == get_java_type_for_python_type(Entity).getJavaClass()
    )
    assert (
        solver_config.getScoreDirectorFactoryConfig().getConstraintProviderClass()
        == my_constraints._timefold_java_class
    )  # noqa
...
def test_reload_from_solver_config_file():
    ...
    solver_config_1 = get_java_solver_config(
        test_directory / "solverConfig-redefined.xml"
    )
    ...
    solver_config_2 = get_java_solver_config(
        test_directory / "solverConfig-redefined.xml"
    )

    assert (
        solver_config_1.getSolutionClass()
        == get_java_type_for_python_type(RedefinedSolution1).getJavaClass()
    )
    assert (
        solver_config_2.getSolutionClass()
        == get_java_type_for_python_type(RedefinedSolution2).getJavaClass()
    )
...
def test_cannot_find_solver_config_file():
    ...
    with pytest.raises(
        FileNotFoundError,
        match=re.escape("The solverConfigFile (does-not-exist.xml) was not found."),
    ):
