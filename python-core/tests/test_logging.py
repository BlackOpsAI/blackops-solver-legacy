...
def assert_in_logs(log_messages: list[str], message: str):
    ...
    raise AssertionError(
        f"Expected message {message} in {log_messages}, but it was not found."
    )


def test_log_events_are_forwarded(caplog):
    with caplog.at_level(logging.INFO, logger="blackops_legacy.solver"):
...
def test_support_changing_log_levels(caplog):
    with caplog.at_level(logging.DEBUG, logger="blackops_legacy.solver"):
        solver_config = SolverConfig(
            solution_class=Solution,
            entity_class_list=[Entity],
            score_director_factory_config=ScoreDirectorFactoryConfig(
...
def test_support_trace_logs(caplog):
    ...
    with caplog.at_level(5, logger="blackops_legacy.solver"):
        solver_config = SolverConfig(
            solution_class=Solution,
            entity_class_list=[Entity],
            score_director_factory_config=ScoreDirectorFactoryConfig(
