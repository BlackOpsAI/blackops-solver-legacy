...
def test_solve_partial():
    ...
    def my_constraints(constraint_factory: ConstraintFactory):
        return [is_value_one(constraint_factory)]
...
def test_solve_nullable():
    ...
    class Entity:
        ...
        value: Annotated[
            Optional[Value],
            PlanningVariable(
                allows_unassigned=True, value_range_provider_refs=["value_range"]
            ),
        ] = field(default=None)

    def at_least_one_null(constraint_factory: ConstraintFactory):
        return (
            constraint_factory.for_each_including_unassigned(Entity)
    ...
    @constraint_provider
    def my_constraints(constraint_factory: ConstraintFactory):
        return [at_least_one_null(constraint_factory), assign_to_v1(constraint_factory)]
    ...
    solver_config = SolverConfig(
        ...
        termination_config=TerminationConfig(best_score_limit="1hard/1soft"),
    )
...
def test_solve_typed():
    ...
    @constraint_provider
    def my_constraints(constraint_factory: ConstraintFactory):
        return [assign_to_v1(constraint_factory)]
    ...
    solver_config = SolverConfig(
        ...
        termination_config=TerminationConfig(best_score_limit="2"),
    )
...
def test_solve_complex_problem_facts():
    ...
    class Entity:
        ...
        value: Annotated[
            Optional[BaseValue],
            PlanningVariable(value_range_provider_refs=["value_range"]),
        ] = field(default=None)

        def get_allowable_values(
            self,
        ) -> Annotated[List[BaseValue], ValueRangeProvider(id="value_range")]:
    ...
    def simultaneous_values(constraint_factory):
        return (
            constraint_factory.for_each_unique_pair(
                ...
                (
                    Joiners.overlapping(
                        lambda entity: entity.start_time, lambda entity: entity.end_time
                    )
                ),
            )
            .filter(
                lambda entity_1, entity_2: (
                    is_present(entity_1.value)
    ...
    solver_config = SolverConfig(
        ...
        termination_config=TerminationConfig(best_score_limit="0hard/0soft"),
    )
...
def test_single_property():
    ...
    def my_constraints(constraint_factory: ConstraintFactory):
        return [
            ...
            .join(
                Value,
                Joiners.equal(lambda entity: entity.value, lambda value: value.code),
            )
            .reward(SimpleScore.ONE)
...
def test_constraint_stream_in_join():
    ...
    problem = Solution(
        [entity_1, entity_2, entity_3, entity_4, entity_5], [value_1, value_2, value_3]
    )
...
def test_tuple_group_by_key():
    ...
    solver_config = SolverConfig(
        ...
        termination_config=TerminationConfig(best_score_limit=str(len(entity_list))),
    )

    problem: Solution = Solution(entity_list, [Value("1")], ["1", "2", "3"])
    solver = SolverFactory.create(solver_config).build_solver()
    solution = solver.solve(problem)
...
def test_custom_planning_id():
    ...
    def my_constraints(constraint_factory: ConstraintFactory):
        return [
            constraint_factory.for_each_unique_pair(
                Entity, Joiners.equal(lambda entity: entity.value)
            )
    ...
    problem: Solution = Solution(
        [entity_1, entity_2, entity_3], [value_1, value_2, value_3]
    )
    solver = SolverFactory.create(solver_config).build_solver()
    solution = solver.solve(problem)
...
def test_custom_comparator():
    ...
    def my_constraints(constraint_factory: ConstraintFactory):
        return [
            ...
            .if_exists_other(
                Entity,
                Joiners.less_than_or_equal(lambda entity: entity.value),
...
def test_custom_equals():
    ...
    def my_constraints(constraint_factory: ConstraintFactory):
        return [
            constraint_factory.for_each_unique_pair(
                Entity, Joiners.equal(lambda entity: entity.value.code)
            )
            .penalize(SimpleScore.ONE)
            .as_constraint("Same value")
        ]
...
def test_entity_value_range_provider():
    ...
    class Entity:
        ...
        value: Annotated[
            Value, PlanningVariable(value_range_provider_refs=["value_range"])
        ] = field(default=None)

        def get_possible_values(
            self,
        ) -> Annotated[List[Value], ValueRangeProvider(id="value_range")]:
    ...
    problem: Solution = Solution([entity_1, entity_2, entity_3])
    solver = SolverFactory.create(solver_config).build_solver()
    solution = solver.solve(problem)
    assert solution.score.score == 0
...
def test_int_value_range_provider():
    ...
    class Entity:
        ...
        actual_value: int
        value: Annotated[
            int, PlanningVariable(value_range_provider_refs=["value_range"])
        ] = field(default=None)
        possible_values: Annotated[
            CountableValueRange, ValueRangeProvider(id="value_range")
        ] = field(init=False)

        def __post_init__(self):
            self.possible_values = ValueRangeFactory.create_int_value_range(
