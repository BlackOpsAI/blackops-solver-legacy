...
from ai.timefold.solver.core.api.score.constraint import (
    ConstraintRef as JavaConstraintRef,
...
class Entity:
    ...
    value: Annotated[int, PlanningVariable] = field(
...
def assert_score_explanation(
    problem: Solution, score_explanation: ScoreExplanation[Solution]
):
    ...
    constraint_ref = ConstraintRef(
        package_name="tests.test_solution_manager", constraint_name="Maximize Value"
    )
            ...
            constraint_match_set={
                ConstraintMatch(
                    ...
                    justification=DefaultConstraintJustification(
                        facts=(entity,), impact=SimpleScore.ONE
                    ),
...
def assert_constraint_analysis(
    problem: Solution, constraint_analysis: ConstraintAnalysis
):
    constraint_ref = ConstraintRef(
        package_name="tests.test_solution_manager", constraint_name="Maximize Value"
    )
...
def assert_solution_manager(solution_manager: SolutionManager[Solution]):
    problem: Solution = Solution(
        [Entity("A", 1), Entity("B", 1), Entity("C", 1)], [1, 2, 3]
    )
...
def test_score_manager_solution_initialization():
    ...
    second_problem: Solution = Solution(
        [Entity("A", None), Entity("B", None), Entity("C", None)], [1, 2, 3]
    )
...
def test_score_manager_diff():
    ...
    score_analysis = solution_manager.analyze(problem)
    second_problem: Solution = Solution(
        [Entity("A", 1), Entity("B", 1), Entity("C", 1), Entity("D", 1)], [1, 2, 3]
    )
    second_score_analysis = solution_manager.analyze(second_problem)
...
def test_score_manager_constraint_analysis_map():
    ...
    constraint_analysis = score_analysis.constraint_analysis(
        "tests.test_solution_manager", "Maximize Value"
    )
...
def test_score_manager_constraint_ref():
    constraint_ref = ConstraintRef.parse_id(
        "tests.test_solution_manager/Maximize Value"
    )
...
ignored_java_functions_per_class = {
    ...
    "ConstraintRef": {
        ...
        "packageName",
        "constraintName",
    },  # built-in constructor and properties with @dataclass
...
def test_has_all_methods():
    ...
    for python_type, java_type in (
        ...
        ignored_java_functions_type = (
            ignored_java_functions_per_class[type_name]
            if type_name in ignored_java_functions_per_class
            else {}
        )
        ...
        for function_name, function_impl in inspect.getmembers(
            ...
            if (
                function_name in ignored_java_functions
                or function_name in ignored_java_functions_type
            ):
                continue
            ...
            snake_case_name_without_prefix = re.sub(
                "(.)([A-Z][a-z]+)",
                r"\1_\2",
                function_name[3:] if function_name.startswith("get") else function_name,
            )
            snake_case_name_without_prefix = re.sub(
                "([a-z0-9])([A-Z])", r"\1_\2", snake_case_name_without_prefix
            ).lower()
            if not hasattr(python_type, snake_case_name) and not hasattr(
                python_type, snake_case_name_without_prefix
            ):
